...
bool SparseHavePathKM(int i, int n, std::vector<std::pair<int, double> > *edges,
                std::vector<double> &slk, std::vector<int> &vis_l,
                std::vector<int> &vis_r, std::vector<double> &mark_l, std::vector<double> &mark_r,
                std::vector<int> &p) {
  vis_l[i] = 1;
  for (const auto &edge : edges[i]) {
    ...
    if (std::abs(cost - mark_l[i] - mark_r[j]) < 1e-3) {
      if (vis_r[j])
        continue;
      vis_r[j] = 1;
      if (p[j] == -1 || SparseHavePathKM(p[j], n, edges, slk, vis_l, vis_r, mark_l, mark_r, p)) {
        p[j] = i;
        // std::cout << "Yes " << i << " " << j << std::endl;
        return true;
      }
...
void SparseMatchingKM(int size, std::vector<std::pair<int, double> > *edges, std::vector<int> &pas) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    while (!SparseHavePathKM(i, n, edges, slk, vis_l, vis_r, mark_l, mark_r, pas)) {
      double tmp = 1e9;
      for (int j = 0; j < n; j++) {
        if (!vis_r[j])
          tmp = std::min(tmp, slk[j]);
